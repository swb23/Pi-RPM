...
def speichern(rpm):
        curs=db.cursor()
        curs.execute("""INSERT INTO Umdrehungen (zeitstempel, wind) VALUES (NOW(), '%s')""" %(rpm) )
        curs.close()
        db.commit()   
...
def main():
    ...
    t1=timer()
    GPIO.add_event_detect(27, GPIO.BOTH, callback=signalerkennung)
    try:
